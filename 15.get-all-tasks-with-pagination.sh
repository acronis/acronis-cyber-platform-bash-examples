...
# Get list of all tasks for all subtenants
...
_after_cursor=$(_get_api_call_bearer "api/task_manager/v2/tasks?limit=${_page_size}" \
...
_after_cursor=$(_get_api_call_bearer "api/task_manager/v2/tasks?limit=${_page_size}&after=${_after_cursor}" \
					 | jq '.paging.cursors.after' | sed -e 's/^"//' -e 's/"$//')

_page_number=$((_page_number+1))
done

echo "The tasks were paged to the end."
